...
describe('tz-parzer', () => {
  ...
  it('should return TZ-AVL05 13 alarm parsed', () => {
    const raw = new Buffer('$$B6869444005480041|13$GPRMC,194329.000,A,3321.6735,S,07030.7640,W,0.00,0.00,090216,,,A*6C|02.1|01.3|01.7|101100001100|20160209194326|13981188|00000000|32D3A03F|0000|0.6376|0100|E54A\r\n');
    const data = tz.parse(raw);
    expect(data.raw).to.eql(raw.toString());
    expect(data.manufacturer).to.eql('tz');
    expect(data.device).to.eql('tz');
